      ...
      .overlay(
        GeometryReader { proxy in
          ...
          .onChange(of: proxy.size) { newValue in
            if maximum {
              size = CGSize(
                width: max(width, newValue.width),
                height: max(height, newValue.height)
              )
              width = size.width
              height = size.height
            } else {
...
extension View {
  func contentSize(_ size: Binding<CGSize>) -> some View {
    self.modifier(
      SizeModifiers(size: size, width: .constant(0), height: .constant(0))
    )
  }
  ...
  func contentSize(height: Binding<CGFloat>, maximum: Bool = false) -> some View
  {
    self.modifier(
      SizeModifiers(
        size: .constant(CGSize(width: 0, height: 0)),
        width: .constant(0),
        height: height,
        maximum: maximum
      )
  ...
  func contentSize(width: Binding<CGFloat>, height: Binding<CGFloat>)
    -> some View
  {
    self.modifier(
      SizeModifiers(
